...
-- Defining dim_customers view for the gold layer
...
SELECT
	...
	CASE 
		WHEN cst_gndr = 'Unknown' AND gen IN ('Male', 'Female') THEN gen -- CRM is the primary source for gender
		ELSE cst_gndr
	END AS gender,
...
-- Defining dim_products view for the gold layer.
...
ON prd.cat_id = px.id
WHERE prd.prd_end_dt IS NULL;  -- Filter Out all historical data
GO
...
-- Defining fact_sales view for the gold layer
